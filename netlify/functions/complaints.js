...
const handleGetRequest = async (event) => {
    try {
        ...
        const tokenValidation = validateToken(bearerToken.slice(7));
...
const handlePostRequest = async (event) => {
    try {
        ...
        if (
            tokenValidation.grant != "admin" &&
            tokenValidation.grant != "occupant" &&
            tokenValidation.grant != "executive" &&
            tokenValidation.grant != "supervisor"
        ) {
            return {
                statusCode: 401,
                body: "Not authorized.",
        ...
        headerRow.forEach((head) => {
            ...
            if (head === "id") {
                newRow.push(newId);
            } else if (plEntry) newRow.push(plEntry[1]);
            else newRow.push("");
        });
        ...
        return {
            ...
            headers,
            body: JSON.stringify({
                uri: `/.netlify/functions/complaints?id=${newId}`,
            }),
        };
    } catch (error) {
        return {
...
const handlePutRequest = async (event) => {
    try {
        ...
        return {
            ...
            headers,
            body: JSON.stringify({
                uri: `/.netlify/functions/complaints?id=${payload.id}`,
            }),
        };
    } catch (error) {
        return {
...
const headersToCamelCase = (headerRow) => {
    ...
    headerRow.forEach((element) => {
        ...
        result.push(
            element.toLowerCase().replace(/[^a-zA-Z0-9]+(.)/g, function (match, chr) {
